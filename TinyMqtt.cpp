...
{
  if (remove->client && remove->connection) remove->connection->closeConnection();
...
MqttError MqttMessage::sendTo(MqttClient* client, int line)
{
  ...
  if (buffer.size())
  {
    ...
    encodeLength();
    // hexdump("snd");
    if (client->connection) client->connection->writeBuffer((byte*)&buffer[0], buffer.size());
